...
void PackData(char *out)
{
    ...
    // ID
    GET_ID(buf);
    sprintf(out + strlen(out), "ID=%s;", buf);

    // Numbers
    for(uint8_t i=0; i<4; i++)
    {
        GET_Number(i, number);
        sprintf(out + strlen(out), "N%d=%s;", i+1, number);
    }

    // Relay
    sprintf(out + strlen(out), "R1=%d;R2=%d;", GET_RelayState1(), GET_RelayState2());

    // Security
    sprintf(out + strlen(out), "S=%d;", GET_SecurityState());

    // Currents
    sprintf(out + strlen(out), "LC=%.2f;HC=%.2f;", GET_LowCurrent(), GET_HighCurrent());

    // Times
    sprintf(out + strlen(out), "CT=%d;RT=%d;", GET_CurrentTime(), GET_ReleaseTime());

    // Modes
    sprintf(out + strlen(out), "RS=%d;AM=%d;DM=%d;",
...
//{
//    // ???? ???? CRC ?? ?????
//    char *crc_pos = strstr(in, "CRC=");
//    if(crc_pos == NULL) return 0; // CRC ????? ????
//
//    uint16_t received_crc;
//    sscanf(crc_pos, "CRC=%4hx;", &received_crc);
//
//    // ???? ???? ???? CRC
//    *crc_pos = '\0';
//
//    // ?????? CRC ??? ???? ????
//    uint16_t calc_crc = CRC16(in, strlen(in));
//
//    if(calc_crc != received_crc)
//    {
//        show_uart("? Data CRC ERROR! EEPROM NOT UPDATED.");
//        return 0; // ???? ???? ???
//    }
//
//    // CRC ???? ??? ? ????????? EEPROM
...
//    }
//
//    show_uart("? EEPROM updated successfully!");
//    return 1;
//}
